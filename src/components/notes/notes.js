...
import { Archive, Tag, Trash } from 'react-bootstrap-icons'
...
function Notes() {
  ...
  async function getNotes() {
    try {
      const response = await axios.get('http://localhost:8001/getnotes?'+new URLSearchParams({
        id:userDetails._id
      }))

      const result = await axios.get('http://localhost:8001/gettrash?'+new URLSearchParams({
        id:userDetails._id
      }))
